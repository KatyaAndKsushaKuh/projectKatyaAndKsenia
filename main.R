...
get_data <- function(url) {
  ...
  xlsfile_1 <- read_xlsx(temp, sheet = 1, skip = 3)
  xlsfile_2 <- read_xlsx(temp, sheet = 2, skip = 2)
...
plot_data <- function(table_plot) {
  ...
  plot1 <- table_plot %>% 
    ...
    geom_line() +
    labs(title = "Динамика реальной среднемесячной ЗП", 
         subtitle = "По регионам России")
  ...
  plot2 <- table_plot %>% 
    ...
    labs(title = "Распределение реальной среднемесячной ЗП", 
         subtitle = "По регионам России")
  ...
  plot3 <- cbind(table_plot[, 1:2], 
    ...
    scale_fill_gradient2(low = "red", high = "green", mid = "white", 
                         ...
                         space = "Lab",
                         name = "Увелчие/Снижение")+
    labs(title = "Изменение реальной среднемесячной ЗП", 
         subtitle = "По регионам России")
